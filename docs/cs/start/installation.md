...
| [User model](../modules/auth.md) | Přidá do `App\Models\User` `TwoFactorAuthenticatable`, pokud je v `config/fortify.php` zapnuté dvoufázové ověření, a `PasskeyAuthenticatable` s `implements PasskeyUser`, pokud jsou zapnuté passkeys. `fortify:install` publikuje funkce i tabulky a model nechá být, takže bez toho přihlašovací obrazovka nabízí passkey, který nikdo nemůže zaregistrovat, a profil nevykreslí ani jednu kartu. Řídí se publikovaným configem, přidá jen to, co zapnutá funkce potřebuje, a nikdy dvakrát |
...
| [Sign-in destination](../panels/routing.md#zony) | Nasměruje `home` Fortify v `config/fortify.php` na vlastní adresu adminu (`/admin`), která člověka pošle dál na první stránku, kterou smí otevřít. Fortify publikuje `/home`, kterou čerstvý Laravel neroutuje, takže bez toho je první přihlášení 404. Nahradí se jen výchozí hodnota Fortify — `home`, kterou jste už změnili, zůstane vaše |
