...
`Foundation\Concerns\HasColor` jsou **dveře**, kterými se ptá každý surface. Ptejte
se jich místo re-enkódování `match` mapy a barva se vyřeší všude stejně.

Samotná pravidla bydlí v pěti třídách pod `Foundation\Colors`, jedna na surface —
...
Při přidávání barvy nebo surface rozšiřte jednou paletu, která ji vlastní — navazující
