...
| [User model](../modules/auth.md) | Puts `TwoFactorAuthenticatable` on `App\Models\User` where two-factor is switched on in `config/fortify.php`, and `PasskeyAuthenticatable` with `implements PasskeyUser` where passkeys are. `fortify:install` publishes the features and the tables and leaves the model alone, so without this the sign-in screen offers a passkey nobody can register and the profile draws neither card. Follows the published config, adds only what a feature that is on needs, and never twice |
...
| [Sign-in destination](../panels/routing.md#zones) | Points Fortify's `home` in `config/fortify.php` at the admin's own address (`/admin`), which sends a person on to the first page they may open. Fortify publishes `/home`, which a fresh Laravel does not route, so without it the first sign-in is a 404. Only Fortify's default is replaced — a `home` you already changed stays yours |
