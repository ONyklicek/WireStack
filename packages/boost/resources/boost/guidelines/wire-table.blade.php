...
- **Resources bind one entity to its surfaces** (`NyonCode\WireTable\Resources\`). A resource declares an entity once — its list, its create/edit form, its read-only view — instead of that wiring being repeated in every Livewire component that shows it. It is **several small contracts, not one**: `DescribesResource` (identity: `key()`, `modelClass()`, `label()`, `pluralLabel()`) plus whichever of `ProvidesResourceTable` / `ProvidesResourceForm` / `ProvidesResourceInfolist` apply, so a read-only log declares identity + table and a page that needs a form cannot be handed one that has none. Identity is **static** and the surfaces are **instance** methods, and that is load-bearing rather than stylistic: a menu asks for a label and `ResourceRegistry::forModel()` routes a model to its owner *before anything is instantiated*, so metadata must answer without an instance, while the surfaces receive a builder the caller already wired to its host (same shape as `RelationManager::table()` and any `WithTable` component) and hand it back. Use the `DescribesRecords` trait and declare only `modelClass()` — key/label/plural derive from the model (`App\Models\OrderLine` → `order-lines` / `Order Line` / `Order Lines`, irregular plurals via Laravel's inflector), so the key the registry routes on and the label a menu shows cannot drift apart. A resource over a non-Eloquent `DataSource` returns `null` from `modelClass()`, is registered and listed normally, and simply is not findable by model. Register in `config('wire-table.resources')`, or `app(ResourceRegistry::class)->register(...)` at runtime; re-registering the same class is a no-op, two different classes on one key throws. It is a registry, **not a panel** — no routing, no URL shell, no navigation tree. One `form()` serves create and edit deliberately; persistence stays the form's (`Form::using()` for non-Eloquent). **`ListPage`** (`Resources\Pages\ListPage`) renders a resource's list: a Livewire component composing `WithTable`, so polling / row partials / gestures / exports all arrive unchanged — set `protected static ?string $resource`, and the heading defaults to the resource's plural label. A page may instead write its own `table()` and use no resource at all; both paths are first class, and a page left *half* declared (no resource **and** no `table()`, or a resource with no `ProvidesResourceTable`) throws rather than rendering an empty table, because empty reads as "no records" instead of as a mistake. Pages own no routing — mount them like any Livewire component.
