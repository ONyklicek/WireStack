---
order: 20
summary: "The production setup: requirements, the install path for each layer, Tailwind and Livewire wiring, assets, and the first table and form."
---
...
Install the layer you need. Each one pulls in what it depends on, and nothing
above it — the full list is in the [Project Map](project-map.md#packages).

### The whole stack, set up for you
...
The interactive installer is the shortest path from a clean Laravel to a working
admin; see [Installing Wire](installation.md) for what it does and what it
deliberately does not. Everything below is the same stack, assembled by hand.

### Tables (table + forms + core)
...
### Resources and their pages (panels + table + forms + core)

```bash
composer require nyoncode/wire-panels
```

### The admin shell (layout and sidebar)
...
Wire components use `primary` as the default accent color (buttons, badges, focus rings, etc.).

**`php artisan wire-admin:install` writes it for you**, mapped to Tailwind's blue,
and leaves an application that already defined `--color-primary-500` alone. Blue
because it reads as *the action* without also meaning something: green, amber and
red are spoken for by success, warning and danger, and an accent that collides
with a signal has to be explained. It is a starting point — one edit and the
whole stack follows.

To choose your own, or to set it up by hand:
...
- [Table columns](../table/columns/index.md) — all 19 column types
- [Form fields](../forms/overview.md) — all field types and Form API
- [Actions](../core/actions/index.md) — row, bulk, header actions
- [Core plugins](../core/plugins/index.md) — reusable app and package extensions
