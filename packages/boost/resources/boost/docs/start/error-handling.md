---
order: 80
summary: "Wire fails loudly and catchably: one final exception class per failure, one marker interface per package, so you catch as broadly as you like."
---
...
| | `InvalidWidgetDataException` | a widget's `items()` is handed an entry of the wrong class |
