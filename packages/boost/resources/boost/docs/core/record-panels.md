---
order: 60
summary: "An infolist you can edit: read-only entries beside switches, selects and inputs that commit straight back to the record, one change at a time."
---

# Editable Panels

A **panel** is an editable "record panel": it looks like an [infolist](infolists/index.md) — the same declarative schema of sections, grids, and entries — but alongside read-only entries it can host **editable** entries (switch, checkbox, select, text input) that write **straight back to the record**. Each change commits on its own with optimistic UI and optimistic locking, the same write path as [editable table columns](../table/columns/editing.md) — no Save button, no form buffer.
...
## Writing Your Own Editable Entry

The four above are `EditableEntry` subclasses, and so is one of yours: the base
class owns the write path — the optimistic update, the version check, the
rollback and the sync node — and leaves the subclass its control's markup and the
value it reads back. That split is the reason a custom entry gets optimistic
locking for free rather than a second implementation of it.
