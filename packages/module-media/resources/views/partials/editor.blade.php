    ...
    <div class="flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-gray-900">
        ...
        <div class="flex min-h-0 flex-1 flex-col md:flex-row">
            ...
            <div class="flex w-full shrink-0 flex-col gap-4 border-b border-gray-200 p-4 md:w-56 md:border-r md:border-b-0 dark:border-gray-800">
                <div>
                    ...
                    <div class="mt-1.5 flex flex-wrap gap-1">
                        <button
                            ...
                            data-testid="media-editor-ratio"
                ...
                <div>
                    ...
                    <div class="mt-1.5 flex gap-1">
                        <button type="button" x-on:click="turn(-90)" data-testid="media-editor-rotate-left" title="{{ __('wire-module-media::messages.rotate_left') }}" class="rounded-lg border border-gray-200 p-1.5 text-gray-500 hover:text-gray-800 dark:border-gray-700">
                            <span class="sr-only">{{ __('wire-module-media::messages.rotate_left') }}</span>
                            {!! icon('outline:arrow-uturn-left', 'h-4 w-4') !!}
                        </button>

                        <button type="button" x-on:click="turn(90)" data-testid="media-editor-rotate-right" title="{{ __('wire-module-media::messages.rotate_right') }}" class="rounded-lg border border-gray-200 p-1.5 text-gray-500 hover:text-gray-800 dark:border-gray-700">
                            <span class="sr-only">{{ __('wire-module-media::messages.rotate_right') }}</span>
                            {!! icon('outline:arrow-uturn-right', 'h-4 w-4') !!}
                        </button>

                        <button type="button" x-on:click="flip = ! flip" data-testid="media-editor-flip" title="{{ __('wire-module-media::messages.flip') }}" :class="flip ? 'border-primary-400 text-primary-700' : 'border-gray-200 text-gray-500'" class="rounded-lg border p-1.5 hover:text-gray-800 dark:border-gray-700">
                ...
                <div>
                    ...
                    <div class="mt-1.5 flex items-center gap-2">
                        <input
                            ...
                            min="16"
                            x-model="targetWidth"
                            data-testid="media-editor-width"
                    ...
                    <select
                        x-model="format"
                        data-testid="media-editor-format"
                ...
                <div class="mt-auto">
                    ...
                    <p class="mt-1 font-mono text-xs text-gray-600 dark:text-gray-300" data-testid="media-editor-result" x-text="outputSize[0] + ' × ' + outputSize[1]"></p>
            ...
            <div class="flex min-h-0 min-w-0 flex-1 flex-col">
                <div class="flex flex-1 items-center justify-center overflow-hidden bg-gray-100 p-4 dark:bg-gray-950">
                    <div class="relative max-h-full" x-ref="stage">
                        ...
                        <div
                            ...
                            data-testid="media-editor-frame"
                        ...
                        >
                            @foreach (['tl' => 'start-0 top-0', 'tr' => 'end-0 top-0', 'bl' => 'start-0 bottom-0', 'br' => 'end-0 bottom-0'] as $handle => $position)
                                <span
                                    ...
                                    data-testid="media-editor-handle"
                ...
                <div class="flex items-start gap-2 border-t border-amber-300 bg-amber-50 px-4 py-2.5 dark:border-amber-900 dark:bg-amber-950/40">
                    {!! icon('outline:exclamation-triangle', 'h-4 w-4 shrink-0 text-amber-600 dark:text-amber-400') !!}
                    <p class="text-xs text-amber-900 dark:text-amber-200" data-testid="media-editor-warning">
                        {{ trans_choice('wire-module-media::messages.replace_warning', $editingUses, ['count' => $editingUses]) }}
                    </p>
                </div>

                <p x-show="error" x-cloak class="border-t border-red-200 bg-red-50 px-4 py-2 text-xs text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300" x-text="error" data-testid="media-editor-error"></p>
                ...
                <div class="flex flex-wrap items-center justify-end gap-2 border-t border-gray-200 px-4 py-3 dark:border-gray-800">
                    <button
                        ...
                        data-testid="media-editor-replace"
                        class="rounded-lg border border-amber-400 px-3 py-1.5 text-sm text-amber-700 hover:bg-amber-50 disabled:opacity-50 dark:text-amber-300 dark:hover:bg-amber-950/40"
                    ...
                    <button
                        ...
                        x-on:click="save('new')"
                        :disabled="busy || tooLarge"
                        data-testid="media-editor-save-new"
