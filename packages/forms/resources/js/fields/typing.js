 ...
 * reads `9. 3. 2026 14:30` while the state holds `2026-03-09T14:30` — so a typed
 * string has to be read back through the same format it was written with. That
 * inverse parser is identical for the two widgets. What differs is what a parsed
 * set of numbers *means* (a calendar day plus a clock, or a clock alone), so each
 * controller supplies its own `applyTyped()`.
 *
 * Two copies of the same parser is the pair that drifts, and a date parser that
 * drifts by one format token stores the wrong day without saying so.
 *
 * Spread into a controller's returned object, so every entry here is a property
 * or a method. The host must provide:
 *   - value              the widget's state
 *   - typedFormat        the format the trigger renders
 *   - applyTyped(parts)  commit {year, month, day, hours, minutes, seconds}
 *                        (whichever subset the format carries) and return true,
 *                        or return false to refuse the whole entry
 */
export const typing = () => ({
    ...
    // renders — the exact inverse of the formatter in the host controller.
    typedTokens() {
        const keys = {
...
/**
 * Render a widget-parseable state string through a PHP `date()` format.
 *
 * Shared by both pickers' `displayValue`. State is always one of Y-m-d,
 * Y-m-d\TH:i, H:i or Y-m, and is read WITHOUT `Date()`, which would drag the
 * browser's timezone into a value that carries none. Tokens the pickers cannot
 * honour pass through, and `\` escapes a literal.
 */
export const formatState = (state, format) => {
    if (! state) return ''
    if (! format) return state

    const [datePart = '', timePart = ''] = String(state).split(/[T ]/)
