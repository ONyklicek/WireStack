@php /** @var \NyonCode\WireForms\Components\Textarea $field */
...
@include('wire-forms::partials.field-wrapper-start')
    ...
    <textarea
        ...
        rows="{{ $field->getRows() }}"
        @if($field->getCols()) cols="{{ $field->getCols() }}" @endif
        @if($field->getPlaceholder()) placeholder="{{ $field->getPlaceholder() }}" @endif
        @if($field->getMinLength()) minlength="{{ $field->getMinLength() }}" @endif
        ...
        @if($field->isAutosize()) x-data="{ resize() { $el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px' } }" x-init="resize()" @input="resize()" @endif
        @class([
