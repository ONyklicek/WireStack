 ...
 *
 * Extracted verbatim from `SlideOverComponent` (Rule 5 framework-wide, Phase 0)
 * so the slide-over shell can be rendered without the Blade component and
 * unit-tested in isolation. Per AI_CODING_STANDARD, presentation logic belongs
 * in a Support value object, not in the component.
 *
 * All class strings are full literals — Tailwind scans this file as text, so an
 * interpolated `{$bp}:{$x}` would never generate its CSS.
 */
final class SlideOverStyle
{
    public function __construct(
        private readonly string $width = 'md',
        private readonly string $position = 'right',
        private readonly bool $bottomSheetOnMobile = false,
    ...
    private function offscreenTranslate(): string
    {
        if ($this->bottomSheetOnMobile) {
