 ...
 */
it('leaves every way of operating a row off in the shipped default', function () {
    // What a table gets without saying anything: no keyboard navigation (which
    // would take the rows into the tab order and answer arrows), no drag sweep
    // (which people find by accident) and no range selection (where a modified
    // click stops meaning a click). What is left needs an invitation of its own
    // anyway.
...
it('reads the project default from config', function () {
    ...
    expect(TableGestures::fromConfig(null)->allowsDragSelect())->toBeFalse()
        ->and(TableGestures::fromConfig(null)->allowsRangeSelection())->toBeFalse()
