...
$screens = [
    ...
    'table-split-columns' => ['title' => 'Wire Table Split Columns', 'subtitle' => 'A split registered under a group name: the header is sortable because a child is, so the click must order by that child and not by the group.', 'component' => TablePreview::class, 'variant' => 'split-columns'],
    ...
    'table-inactive-rows' => ['title' => 'Wire Table Inactive Rows', 'subtitle' => 'A deactivated user: the row is dimmed and struck through, every editable cell on it refuses to open, its checkbox is inert — and the action that reactivates it still works.', 'component' => TablePreview::class, 'variant' => 'inactive-rows'],
    ...
    'widgets-progress-list' => ['title' => 'Wire Core Progress &amp; List Widgets', 'subtitle' => 'The two pure-CSS widget types: a fill against a target and a short feed of records, with header actions, a filter and an empty state.', 'component' => WidgetPreview::class, 'variant' => 'progress-list'],
    'widgets-editable' => ['title' => 'Wire Core Widgets · Customisable', 'subtitle' => 'A dashboard the user rearranges: an explicit edit mode, drag to reorder through Livewire\'s own x-sort, size steppers, and save / cancel / reset.', 'component' => WidgetPreview::class, 'variant' => 'editable'],
    'widgets-editable-filtered' => ['title' => 'Wire Core Widgets · Customisable, filtered', 'subtitle' => 'A customisable dashboard under a dashboard filter: the "not filtered" mark and the edit toolbar beside a card header that carries a link.', 'component' => WidgetPreview::class, 'variant' => 'editable-filtered'],
    'widgets-interactive' => ['title' => 'Wire Core Widgets · Interactive', 'subtitle' => 'A filtered list, a deferred progress board and a filtered chart on one grid: the three things a widget can do that only a browser can prove.', 'component' => WidgetPreview::class, 'variant' => 'interactive'],
...
$fieldPreviews = [
    ...
    'morph-to-select' => 'Morph To Select',
    'key-value' => 'Key-Value',
...
// Two endpoints the browser driver reads, because a command that "just runs" has
// no visible outcome to assert against. `InvoiceResource::commands()` writes to
// the cache; these let the driver clear that mark and then watch it appear, which
// is the difference between proving the action ran and proving `select()` did not
// throw on the way to doing nothing.
Route::get('/previews/global-search/recounted', fn (): string => cache()->get('workbench.invoices.recounted') ? 'yes' : 'no');
Route::get('/previews/global-search/forget-recount', function (): string {
    cache()->forget('workbench.invoices.recounted');
