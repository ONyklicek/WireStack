...
/*
 * Phase 3 rollout: the generic <x-wire::dropdown> opens as a bottom sheet on a
...
const shotDir = process.env.SHOT_DIR ?? join(tmpdir(), 'wire-dropdown-shots');
await mkdir(shotDir, { recursive: true });

const userDataDir = join(tmpdir(), `wire-dropdown-${Date.now()}`);
...
try {
  ...
  const openAndProbe = async (w, h) => {
    ...
    await page('Page.navigate', { url: `${base}/core-dropdown` });
    await sleep(2600);
    await eval_(`[...document.querySelectorAll('button')].find(b => /Options/.test(b.innerText))?.click()`);
    await sleep(700);
    return JSON.parse(await eval_(`(() => {
      const panel = [...document.querySelectorAll('[x-ref="panel"]')].find(p =>
        getComputedStyle(p).display !== 'none' && /Duplicate/.test(p.innerText));
  ...
  await shot('01-dropdown-sheet-390');
  check('dropdown opens on mobile', m.open === true);
  check('dropdown is a fixed bottom sheet', m.open && m.position === 'fixed' && Math.abs(m.rect.bottom - m.vh) <= 2, `pos=${m.position} bottom=${m.rect?.bottom} vh=${m.vh}`);
  check('dropdown sheet is full-width', m.open && m.rect.left <= 1 && Math.abs(m.rect.right - m.vw) <= 2, `left=${m.rect?.left} right=${m.rect?.right} vw=${m.vw}`);
  ...
  await shot('02-dropdown-floating-1400');
  check('dropdown opens on desktop', d.open === true);
  check('dropdown is an absolute floating panel', d.open && d.position === 'absolute', `pos=${d.position}`);
  check('desktop dropdown is not full-width', d.open && d.rect.w < 400, `w=${d.rect?.w}`);
