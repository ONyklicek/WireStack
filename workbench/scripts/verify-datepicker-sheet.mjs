...
/*
 * Phase 3 rollout: the date-time-picker calendar opens as a bottom sheet on a
...
const shotDir = process.env.SHOT_DIR ?? join(tmpdir(), 'wire-datepicker-shots');
await mkdir(shotDir, { recursive: true });

const userDataDir = join(tmpdir(), `wire-datepicker-${Date.now()}`);
...
try {
  ...
  const openAndProbe = async (w, h) => {
    ...
    await sleep(2600);
    await eval_(`(document.querySelector('[x-ref="trigger"] input') || document.querySelector('input'))?.click()`);
    await sleep(800);
  ...
  await shot('01-datepicker-sheet-390');
  check('calendar opens on mobile', m.open === true);
  check('calendar is a fixed bottom sheet', m.open && m.position === 'fixed' && Math.abs(m.rect.bottom - m.vh) <= 2, `pos=${m.position} bottom=${m.rect?.bottom} vh=${m.vh}`);
  check('calendar sheet is full-width', m.open && m.rect.left <= 1 && Math.abs(m.rect.right - m.vw) <= 2, `left=${m.rect?.left} right=${m.rect?.right} vw=${m.vw}`);
  ...
  await shot('02-datepicker-floating-1400');
  check('calendar opens on desktop', d.open === true);
  check('calendar is an absolute floating panel', d.open && d.position === 'absolute', `pos=${d.position}`);
  check('desktop calendar is a trigger-anchored panel, not a full-viewport sheet', d.open && d.rect.left > 1 && d.rect.w < d.vw - 50, `left=${d.rect?.left} w=${d.rect?.w} vw=${d.vw}`);
