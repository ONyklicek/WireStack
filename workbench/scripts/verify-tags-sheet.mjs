...
/*
 * Phase 3 rollout (final surface): the tags suggestions list opens as a bottom
 * sheet on a phone (fixed, bottom-pinned, full-width, backdrop) and as a
 * trigger-anchored floating panel on desktop. See .claude/skills/verify-preview.
 */

const base = process.env.PREVIEW_BASE ?? 'http://127.0.0.1:8085/previews';
const chromeBin = process.env.CHROME_BIN
  ?? '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const devtoolsPort = Number(process.env.CHROME_PORT ?? 9344);
const shotDir = process.env.SHOT_DIR ?? join(tmpdir(), 'wire-tags-shots');
await mkdir(shotDir, { recursive: true });

const userDataDir = join(tmpdir(), `wire-tags-${Date.now()}`);
...
try {
  ...
  const openAndProbe = async (w, h) => {
    ...
    await page('Page.navigate', { url: `${base}/field-tags` });
    await sleep(2600);
    // Focus the tag input and type a letter that matches a suggestion so the
    // showDropdown getter (focused && filteredSuggestions.length) turns true.
    await eval_(`(() => {
  ...
  await shot('01-tags-sheet-390');
  check('tags list opens on mobile', m.open === true);
  check('tags list is a fixed bottom sheet', m.open && m.position === 'fixed' && Math.abs(m.rect.bottom - m.vh) <= 2, `pos=${m.position} bottom=${m.rect?.bottom} vh=${m.vh}`);
  check('tags sheet is full-width', m.open && m.rect.left <= 1 && Math.abs(m.rect.right - m.vw) <= 2, `left=${m.rect?.left} right=${m.rect?.right} vw=${m.vw}`);
  ...
  await shot('02-tags-floating-1400');
  check('tags list opens on desktop', d.open === true);
  check('tags list is an absolute floating panel', d.open && d.position === 'absolute', `pos=${d.position}`);
  check('desktop tags list is a trigger-anchored panel, not a full-viewport sheet', d.open && d.rect.left > 1 && d.rect.w < d.vw - 50, `left=${d.rect?.left} w=${d.rect?.w} vw=${d.vw}`);
