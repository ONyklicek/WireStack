...
const shotDir = process.env.SHOT_DIR ?? join(tmpdir(), 'wire-select-shots');
...
try {
  ...
  const openAndProbe = async (w, h) => {
    ...
    await page('Page.navigate', { url: `${base}/field-select` });
    await sleep(2600);
  ...
  await shot('01-select-sheet-390');
  ...
  await shot('02-select-floating-1400');
