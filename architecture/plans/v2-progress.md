...
| Paleta ⌘K zná jen záznamy — chybí navigace a akce | rozhodnutí, **kdo globální příkaz spustí**, a předtím, **jestli je příkazový řádek `NavigationItem`**. Zadání vlastníka 2026-09-08: čtyři druhy řádků (záznamy · navigace · globální příkazy · akce nad záznamem). Navigace je levná — `Workspace::items()` vrací už resolvované položky, je v L1, a `Hook::NavigationBuilding` (`Workspace.php:242`) do té mapy **už umí přidávat řádky**, takže mechanismus vkládání existuje. Akce ne: `GlobalSearch/` i `Actions/` jsou obojí L2, a u globálního příkazu bez tabulky na obrazovce dnes žádný vlastník spouštění neexistuje. `Hook::SearchQuerying` na to neodpovídá — je nad `Builder`em, mění nalezené záznamy, ne druhy řádků. Oba hooky jsou commitnuté (`a5783df9`) a ověřené proti HEAD. Rozepsané v [`v2-master-plan.md`](v2-master-plan.md) § V2.5 |
